...
const genderConfig: Record<string, { label: string; badge: string; backBadge: string }> = {
  MALE: { label: 'Male', badge: 'bg-blue-100 text-blue-700 border-blue-200', backBadge: 'bg-blue-500/20 text-blue-300 border-blue-400/30' },
  FEMALE: { label: 'Female', badge: 'bg-purple-100 text-purple-700 border-purple-200', backBadge: 'bg-purple-500/20 text-purple-300 border-purple-400/30' },
  UNKNOWN: { label: 'Unknown', badge: 'bg-amber-100 text-amber-700 border-amber-200', backBadge: 'bg-amber-500/20 text-amber-300 border-amber-400/30' },
};
      ...
      <div className="relative h-full w-full transition-transform duration-400 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
        ...
        <div className="absolute inset-0 overflow-hidden rounded-2xl shadow-sm [backface-visibility:hidden]">
          ...
          <div className="absolute inset-x-0 top-3 z-10 flex items-center justify-between px-4">
            ...
            <span className={`rounded-full border px-2.5 py-0.5 text-xs font-semibold ${gender.badge} dark:border-slate-600 dark:bg-slate-700/60 dark:text-gray-300`}>
              {gender.label}
            </span>
          </div>
          ...
          <div className="relative z-10 flex h-full flex-col justify-between p-5">
            <div>
              ...
              <div className="mt-2 flex items-center gap-2">
                ...
                <span className={`rounded-full border px-3 py-1 text-xs font-semibold ${gender.badge}`}>
                  {gender.label}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              <Image src={character.image} alt={character.name} width={40} height={40} className="rounded-full" />
